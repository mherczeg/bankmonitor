...
(see [design-decisions.md](design-decisions.md) §6, §13).
...
**Deferred.** The concurrency tests run on H2 (design-decisions.md §25), which
...
**Deferred.** Playwright runs *end-to-mock* (design-decisions.md §24): a real
...
**Deferred.** Timeouts and bounded retries only (design-decisions.md §27).
...
`npm run dev`, with Vite proxying `/api` (design-decisions.md §20).
...
**Deferred.** `openapi-typescript` output is generated by hand and committed
(design-decisions.md §26).

**Why deferred:** a CI pipeline does not exist for this build, so the
enforcement point does not exist either.
...
a non-empty diff. Until then, a README line naming the regeneration command is
