...
`spring.mvc.problemdetails.enabled=true`; responses are
`application/problem+json`. **The `type` URN is the sole discriminator** — not
`type` *and* a `code` field, because two discriminators drift and eventually one
of them lies.
